...
/**
 * Defines the CommonCartridge class, which is meant to contain the CommonCartridge items (nuclear elements)
 * @package chamilo.learnpath
 * @author    Yannick Warnier <ywarnier@beeznest.org>
 */
class CommonCartridge extends learnpath
{
    ...
    /*  Path between the cc/ directory and the imsmanifest.xml e.g.
    ...
    {
        if ($this->debug > 0) {
            error_log('New LP - CommonCartridge::cc('.$course_code.','.$resource_id.','.$user_id.') - In CC constructor', 0);
        }
    ...
    {
        if ($this->debug > 0) {
            error_log('New LP - CommonCartridge::open() - In CommonCartridge::open method', 0);
        }
     ...
     * @param	string	Path to the imsmanifest.xml file on the system. If not defined, uses the base path of the course's CommonCartridge dir
     * @return	array	Structured array representing the imsmanifest's contents
     */
    public function parse_manifest($file = '')
    {
        if ($this->debug > 0) {
            error_log('In CommonCartridge::parse_manifest('.$file.')', 0);
        }
        if (empty($file)) {
        ...
        if (is_file($file) && is_readable($file) && ($xml = @file_get_contents($file))) {
            ...
            if ($this->debug > 0) { error_log('In CommonCartridge::parse_manifest() - Parsing using PHP5 method', 0); }
            ...
            if ($res === false) {
                if ($this->debug > 0) {
                    error_log('New LP - In CommonCartridge::parse_manifest() - Exception thrown when loading '.$file.' in DOMDocument', 0);
                }
                // Throw exception?
                return null;
            }
    ...
    private function detect_manifest_encoding(& $xml)
    {
        ...
        */
        if (preg_match_all('/<lomimscc:title[^>]*>(.*)<\/lomimscc:title>/m', $xml, $matches)) {
            $test_string .= "\n".implode("\n", $matches[1]);
            unset($matches);
        }
    ...
    /**
     * Import the CommonCartridge object (as a result from the parse_manifest function) into the database structure
     * @param int $cId
     ...
     */
    public function import_manifest($cId, $userMaxScore = 1, $sessionId = 0, $userId = 0)
    {
        if ($this->debug > 0) {
            error_log('New LP - Entered import_manifest('.$cId.')', 0);
        }
        ...
        foreach ($this->organizations as $id => $dummy) {
            ...
            // -for items
            // -for views?
            $get_max = "SELECT MAX(display_order) FROM $new_lp WHERE c_id = $cId ";
            ...
            foreach ($list as $item) {
                ...
                $sql = "INSERT INTO $new_lp_item (c_id, lp_id,item_type,ref,title, path,min_score,max_score, $field_add parent_item_id,previous_item_id,next_item_id, prerequisite,display_order,launch_data, parameters)
                        VALUES ($cId, $lp_id, '$type', '$identifier', '$title', '$path' , 0, $max_score, $value_add $parent, $previous, 0, '$prereq', ".$item['rel_order'].", '".$item['datafromlms']."', '".$item['parameters']."' )";
                ...
                if ($item_id) {
                    ...
                    $upd = "UPDATE $new_lp_item SET next_item_id = $item_id
                            WHERE c_id = $cId AND id = $previous";
                    Database::query($upd);
                ...
                if (!empty($_POST['index_document'])) {
                    ...
                    if ($did) {
                        ...
                        $sql = sprintf($sql, $tbl_se_ref, $courseInfo['code'], TOOL_LEARNPATH, $lp_id, $previous, $did);
                        Database::query($sql);
                    }
                }
            }
     ...
     * @param  string  $file_path  Path to the zip file, from the sys root
     * @param  string  $current_dir  Current path (optional)
    ...
    {
        if ($this->debug > 0) {
            error_log('In CommonCartridge::import_package('.print_r($zip_file_info, true).',"'.$current_dir.'") method', 0);
        }
        ...
        $course_rel_dir = api_get_course_path($courseInfo['directory']).'/cc'; // cc dir web path starting from /courses
        $course_sys_dir = api_get_path(SYS_COURSE_PATH).$course_rel_dir; // Absolute system path for this course.
        $current_dir = api_replace_dangerous_char(trim($current_dir)); // Current dir we are in, inside cc/
        ...
        foreach ($zipContentArray as $thisContent) {
            ...
            if (preg_match('~.(php.*|phtml)$~i', $thisContent['filename'])) {
                ...
                $manifest = $thisContent['filename']; //just the relative directory inside cc/
            } else {
                // Do nothing, if it has not been set as cc somewhere else, it stays as '' default.
            }
        ...
        if ($package_type == '') {
            // && defined('CHECK_FOR_COMMON_CARTRIDGE') && CHECK_FOR_COMMON_CARTRIDGE)
            if ($this->debug > 1) { error_log('New LP - Package type is empty', 0); }
            Display::addFlash(
                Display::return_message(get_lang('NotCommonCartridgeContent'))
            );
            ...
            if ($dir = @opendir($course_sys_dir.$new_dir)) {
                ...
                while ($file = readdir($dir)) {
                    if ($file != '.' && $file != '..') {
                        ...
                        // TODO: RENAMING FILES CAN BE VERY DANGEROUS CC-WISE, avoid that as much as possible!
    ...
    public function set_proximity($proxy = '', $courseId = null)
    {
        ...
        if ($this->debug > 0) { error_log('In CommonCartridge::set_proximity('.$proxy.') method', 0); }
        $lp = $this->get_id();
        if ($lp != 0) {
    ...
    public function set_theme($theme = '')
    {
        ...
        if ($this->debug > 0) { error_log('In CommonCartridge::set_theme('.$theme.') method', 0); }
        $lp = $this->get_id();
        if ($lp != 0) {
            $tbl_lp = Database::get_course_table(TABLE_LP_MAIN);
    ...
    {
        $courseId = api_get_course_int_id();
        if ($this->debug > 0) { error_log('In CommonCartridge::set_theme('.$preview_image.') method', 0); }
        $lp = $this->get_id();
        if ($lp != 0) {
            $tbl_lp = Database::get_course_table(TABLE_LP_MAIN);
    ...
    {
        $courseId = api_get_course_int_id();
        if ($this->debug > 0) { error_log('In CommonCartridge::set_author('.$author.') method', 0); }
        $lp = $this->get_id();
        if ($lp != 0) {
            $tbl_lp = Database::get_course_table(TABLE_LP_MAIN);
     ...
     * @param string  $maker  Proximity setting
    ...
    {
        $courseId = empty($courseId) ? api_get_course_int_id() : intval($courseId);
        if ($this->debug > 0) { error_log('In CommonCartridge::set_maker method('.$maker.')', 0); }
        $lp = $this->get_id();
        if ($lp != 0) {
            $tbl_lp = Database::get_course_table(TABLE_LP_MAIN);
    ...
    /**
     * Exports the current CommonCartridge object's files as a zip.
    ...
    {
        if ($this->debug > 0) { error_log('In CommonCartridge::export_zip method('.$lp_id.')', 0); }
        if (empty($lp_id)) {
            if (!is_object($this)) {
                return false;
            } else {
        ...
        //zip everything that is in the corresponding CommonCartridge dir
        ...
        $ccfoldername = api_get_path(SYS_COURSE_PATH).$_course['directory'].'/cc/'.$LPnamesafe;
        ...
        $zip_folder->create($ccfoldername.'/', PCLZIP_OPT_REMOVE_PATH, $ccfoldername.'/');
    ...
    {
        if ($this->debug > 0) { error_log('In CommonCartridge::get_res_path('.$id.') method', 0); }
        $path = '';
    ...
    {
        if ($this->debug > 0) { error_log('In CommonCartridge::get_res_type('.$id.') method', 0); }
        $type = '';
        if (isset($this->resources[$id])) {
            $oRes = & $this->resources[$id];
            $temptype = $oRes->get_scorm_type();
    ...
    {
        if ($this->debug > 0) { error_log('In CommonCartridge::get_title() method', 0); }
        $title = '';
    ...
    {
        if ($this->debug > 0) { error_log('In CommonCartridge::reimport_manifest() method', 0); }
        ...
        if ($this->debug > 2) { error_log('New LP - CommonCartridge::reimport_manifest() '.__LINE__.' - Querying lp: '.$sql, 0); }
        $res = Database::query($sql);
        ...
        $manifest_file = api_get_path(SYS_COURSE_PATH).$courseInfo['directory'].'/cc/'.$this->subdir.'/imsmanifest.xml';
        if ($this->subdir == '') {
            $manifest_file = api_get_path(SYS_COURSE_PATH).$courseInfo['directory'].'/cc/imsmanifest.xml';
        }
        echo $manifest_file;
        if (is_file($manifest_file) && is_readable($manifest_file)) {
            // Re-parse the manifest file.
            if ($this->debug > 1) { error_log('New LP - In CommonCartridge::reimport_manifest() - Parsing manifest '.$manifest_file, 0); }
            $manifest = $this->parse_manifest($manifest_file);
            // Import new LP in DB (ignore the current one).
            if ($this->debug > 1) { error_log('New LP - In CommonCartridge::reimport_manifest() - Importing manifest '.$manifest_file, 0); }
            $this->import_manifest($this->course_int_id);
        } else {
            if ($this->debug > 0) { error_log('New LP - In CommonCartridge::reimport_manifest() - Could not find manifest file at '.$manifest_file, 0); }
        }
        return false;
    }
}
