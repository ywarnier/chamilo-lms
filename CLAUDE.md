...
Chamilo HR 2.0 — a talent management platform developed on top of the Chamilo e-learning platform, itself built on **Symfony 6.4** (PHP 8.2/8.3) with a **Vue 3** frontend. Uses **API Platform 3.0** for REST/GraphQL APIs, **Doctrine ORM** for persistence, and **Webpack Encore** for asset compilation.
...
It is possible to test the application through the web, as admin, by calling locally: http://my.skillms.net with credentials admin/admin.
Other user profiles are available like HR manager (hr/HrHrHr11+) and student (student/student11+)
...
```bash
vendor/bin/behat tests/behat/features/hr/hrBranches.feature
```

Run all HR features:
```bash
vendor/bin/behat --suite=default tests/behat/features/hr/
```
...
A MySQL 'chamilohr' database is available to access to verify data as user chamilohr/chamilohr.
...
2. `assets/vue/router/skill.js` (or the relevant domain router file) — so the Vue Router handles it client-side.
...
1. Set `meta: { showBreadcrumb: true, breadcrumb: "Page title" }` on the route entry in the relevant router file (e.g. `assets/vue/router/hr.js`).
2. If the page lives under a new top-level path (not `/admin/*`), add the path prefix to the `whitelist` array in `buildManualBreadcrumbIfNeeded` inside `Breadcrumb.vue`, and add a corresponding `if` block that pushes an "Administration" crumb (linked to `AdminIndex`) followed by the page label. See the `/hr/*` case as a reference.
...
### Migrating a legacy feature to Vue

See `.claude/commands/restore-legacy-feature.md` for the step-by-step checklist, invokable as `/restore-legacy-feature.md` in Claude Code. This covers larger restorations than just pages (as /legacy-to-vue would do).
